...
bash -lc "export PATH=$PATH:$PREFIX/Library/bin && autoreconf -fi && ./configure --build=x86_64-w64-mingw32 --host=x86_64-w64-mingw32 --enable-python-integration --disable-libxrl --disable-fortran2003 --prefix=`cygpath -u $PREFIX` PYTHON=`which python` SWIG=`which swig` LIBS=-L$PWD && make && make install"
