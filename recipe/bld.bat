...
bash -lc "./configure --disable-static --build=$GCC_ARCH --host=$GCC_ARCH --target=$GCC_ARCH --enable-python-integration --disable-perl --disable-lua --disable-ruby --disable-fortran2003 --enable-python --prefix=`cygpath -u $LIBRARY_PREFIX` PYTHON=`which python` SWIG=`which swig` CPPFLAGS=$EXTRA_FLAGS LIBS=-L$PWD"
bash -lc "make"
