...
bash -lc "export PATH=$PATH:$PREFIX/Library/bin && autoreconf -fi && ./configure --build=$GCC_ARCH --host=$GCC_ARCH --target=$GCC_ARCH --enable-python-integration --disable-fortran2003 --enable-python --prefix=`cygpath -u $PREFIX` --bindir=`cygpath -u $LIBRARY_BIN` --libdir=`cygpath -u $LIBRARY_LIB` PYTHON=`which python` SWIG=`which swig` CPPFLAGS=-DMS_WIN64 LIBS=-L$PWD && make && make install"
bash -lc "rm -r $PREFIX/include/xraylib"
bash -lc "rm -f $PREFIX/bin/xraylib"
bash -lc "rm -f $LIBRARY_BIN/xraylib"
bash -lc "rm $SP_DIR/*.la"
bash -lc "rm $SP_DIR/*.dll.a"
bash -lc "rm $SP_DIR/xrayhelp.py*"
bash -lc "rm $SP_DIR/xraymessages.py*"
