...
bash -lc "export PATH=$PATH:$PREFIX/Library/bin && autoreconf -fi && ./configure --build=x86_64-w64-mingw32 --host=x86_64-w64-mingw32 --enable-python-integration --disable-libxrl --prefix=`cygpath -u $PREFIX` PYTHON=`which python` && make && make install"
